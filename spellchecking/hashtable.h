...
#include <ctype.h>
#include <sstream>
#define tablesize 69997
using namespace std;


class hashtable{
private:
    unsigned int collision = 0;       // to detect the number of collisions the search or insert does
    string hasharray[tablesize]; // hashtable is defined
    unsigned int elements = 0;  // to check whether the table is full
public:
    ...
    void insert(string key){
        ...
        else if (hasharray[k] == "" or hasharray[k] == "delete" )
        {
            hasharray[k] = key;
            elements = elements +1;
            return;
